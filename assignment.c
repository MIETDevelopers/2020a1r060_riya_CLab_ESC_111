...
#include<stdlib.h>//Header file for exit() and system()
...
void min()//function to finding the minimum value of walnut
{
	...
	char *sw,*sa;//variable declaration
	...
	{
		sw=temp->shop;//initialising s with the name of shop which is stored in the shop member of temp node
		...
		sa = temp->shop;//initialising sa with the name of shop which is stored in the shop member of temp node
		while(temp!=NULL)//traversing the list till the end
		{
			...
			{
				sw=temp->shop;// then teh value of s, a and min will move to the shop which has the price of walnut less than the previous shop
				
				min=temp->walnuts;
			}
            if (a>temp->Almonds)//if the price of min i.e. price of Almonds is greater than the price of the Almonds of next shop
			{
				sa=temp->shop;// then teh value of s, a and min will move to the shop which has the price of Almonds less than the previous shop
				
				a=temp->Almonds;
			}
			temp=temp->next;
		}   printf("_______\n");
			printf("Shop name selling walnuts minimun :%s\n",sw);//printing the name of the shop with the cheapest price of walnuts
			printf("Cheapest price of walnuts:%d/kg\n",min);//printing the price of walnuts with the cheapest price
            printf("\n&& \n");
            printf("\nShop name selling almonds minimun :%s\n",sa);//printing the name of the shop with the cheapest price of Almonds
